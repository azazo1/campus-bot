...
UPDATE_CAPTCHA_SELECTOR = "#login-normal > div > form > app-verification > nz-input-group > span.ant-input-suffix.ng-star-inserted > div > a"
...
def submit_login(driver: Edge, stu_number: str, password: str, timeout=24 * 60):
    ...
    wait_for(driver, UPDATE_CAPTCHA_SELECTOR, timeout)
    ...
    project_logger.debug(f"parse captcha result: \"{captcha}\".")
    if len(captcha) != 4:
        driver.refresh()
        submit_login(driver, stu_number, password, timeout)
        return
