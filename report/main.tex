...
\begin{tikzpicture}
    ...
    % 在右侧添加描述信息
    \node[anchor=west] at (5, 0) { % 调整 x 偏移量以确定文本位置
        \begin{minipage}{0.45\textwidth} % 调整 minipage 宽度以适应文本框大小
            \textbf{项目结构：}\\
            图中展示了项目的层次结构，主要组件包括：
            \begin{itemize}
                \item \textbf{assets:} 静态资源目录，包含各开发参考文件及图标等。
                \item \textbf{plugins:} 插件主目录，包含 \texttt{electric\_bill}、\texttt{studyroom} 和 \texttt{library} 等模块。
                \item \textbf{src:} 主源代码目录，按照功能分为子模块，例如 \texttt{gui}、\texttt{portal}、\texttt{uia} 等。
                \item \textbf{tests:} 测试用例目录，包含对各模块的单元测试。
                \item \textbf{main.py:} 项目主入口。
            \end{itemize}
        \end{minipage}
    };
\end{tikzpicture}
