...
\makeatletter
	...
	% 答题卡
	\newtcbtheorem[use counter=hitung, number within=section]{ans}{双端通信}
	{theorem style=theorem wide name and number,breakable,enhanced,arc=3.5mm,outer arc=3.5mm,
		boxrule=0pt,toprule=1pt,leftrule=0pt,bottomrule=1pt, rightrule=0pt,left=0.2cm,right=0.2cm,
		titlerule=0.5em,toptitle=0.1cm,bottomtitle=-0.1cm,top=0.2cm,
		colframe=white!10!biru,
...
% 定义节点和箭头样式
\tikzstyle{node_style} = [rectangle, rounded corners, minimum width=3cm, minimum height=1cm, text centered, draw=black, fill=blue!30]
\tikzstyle{server_node_style} = [rectangle, rounded corners, minimum width=3cm, minimum height=1cm, text centered, draw=black, fill=green!30]
\tikzstyle{arrow_style} = [thick,->,>=stealth]
...
\begin{mdframed}
    本模块位于 \texttt{/src/plugins/library} 中。
\end{mdframed}
...
\begin{figure}[H]
    ...
    \begin{minipage}[H]{0.5\textwidth}
        ...
        \begin{forest}
            for tree={
                ...
                parent anchor=east,
                child anchor=west,
                delay={where content={}{shape=coordinate}{}} % 避免空节点
            }
            [electric\_bill
                [\_\_init\_\_.py]
                [bill\_plugin.py]
    ...
    \begin{minipage}[H]{0.45\textwidth}
        ...
        \texttt{bill\_plugin.py}\\
        实现电费账单的生成和管理，包含账单计算逻辑、数据处理和存储功能。
        \vspace{0.25cm}

        \texttt{client.py}\\
        负责与服务器的通信，处理客户端请求和响应。包括建立WebSocket连接、发送和接收数据，以及处理用户交互。
        \vspace{0.25cm}

        \texttt{encryption.py}\\
        提供数据的加密与解密功能，使用AES加密算法保障数据传输的安全性。
        \vspace{0.25cm}

        \texttt{visualize\_degree.py}\\
        实现电量使用情况的可视化展示，利用图形库绘制用电趋势图和账单分布图。
    \end{minipage}
    }
    \caption{ECNU宿舍电费查询插件结构树形图与模块功能说明}
    \label{fig:directory_structure_electric_bill}
\end{figure}
...
\subsubsection{电费查询原理}

与上述模块的流程相似，先获取 \texttt{EpayCache}，之后就可以随时访问 API 获取电费信息。

\begin{ans}{}{}

\begin{itemize}
    \item 客户端负责与用户交互、获取电费信息并向服务器发送相关数据；
    \item 服务器端负责接收客户端的数据请求，处理电费查询，并将结果反馈给客户端。
    \item 数据在传输过程中采用AES加密，确保通信的安全性。
\end{itemize}
...
\begin{center}
	\begin{tikzpicture}[node distance=2cm and 3cm]
	  ...
	  % 第一行
	  \node[node_style] (node0) {客户端启动};
	  \node[node_style, right of=node0, xshift=3cm] (node1) {加载配置文件};
	  \node[node_style, right of=node1, xshift=3cm] (node2) {建立WebSocket连接};
	  \draw[arrow_style] (node0.east) -- (node1.west); 
	  \draw[arrow_style] (node1.east) -- (node2.west);
	  
	  % 第二行
	  \node[node_style, below of=node1, yshift=-0.1cm] (node3) {认证与授权};
	  \node[node_style, right of=node3, xshift=3cm] (node4) {电量查询与监控};
...
\begin{center}
	\begin{tikzpicture}[node distance=2cm and 3cm]
	  ...
	  % 第一行
	  \node[node_style] (node0) {服务器启动};
	  \node[node_style, right of=node0, xshift=3cm] (node1) {加载配置与初始化};
	  \node[node_style, right of=node1, xshift=3cm] (node2) {启动WebSocket服务器};
	  \draw[arrow_style] (node0.east) -- (node1.west); 
	  \draw[arrow_style] (node1.east) -- (node2.west);
	  
	  % 第二行
	  \node[node_style, below of=node1, yshift=-0.1cm] (node3) {处理客户端请求};
	  \node[node_style, right of=node3, xshift=3cm] (node4) {电量查询与记录};
	  \draw[arrow_style] (node2.south) -- ++(0,-0.5) -- ++(-2.5,0) -- (node3.north);
	  \draw[arrow_style] (node3.east) -- (node4.west);
	\end{tikzpicture}
\end{center}
...
\begin{itemize}
    \item 解析客户端发送的加密命令，根据\texttt{type}字段执行相应操作。
    \item 支持的命令包括：
    \begin{itemize}
        \item \texttt{POST\_TOKEN}：接收并存储客户端的CSRF Token和Cookies。
        \item \texttt{POST\_ROOM}：接收并存储宿舍信息（宿舍号、电区、电楼）。
        \item \texttt{GET\_DEGREE}：返回当前剩余电量。
        \item \texttt{FETCH\_DEGREE\_FILE}：返回最近的电量记录文件内容。
    \end{itemize}
\end{itemize}
